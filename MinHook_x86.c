...
static MH_STATUS EnableHookLL(UINT pos, BOOL enable)
{
    ...
    else
    {
        if (pHook->patchAbove)
        {
            ...
            for (size_t i = 0; i < sizeof(JMP_REL) + sizeof(JMP_REL_SHORT); i++)
            {
                ((BYTE*)pPatchTarget)[i] = ((BYTE*)(pHook->backup))[i];
            }
        ...
        else
        {
            ...
            for (size_t i = 0; i < sizeof(JMP_REL); i++)
            {
                ((BYTE*)pPatchTarget)[i] = ((BYTE*)(pHook->backup))[i];
            }
        }
                ...
                if (pBuffer != NULL)
                {
                    ...
                    if (CreateTrampolineFunction(&ct))
                    {
                        ...
                        if (pHook != NULL)
                        {
                            ...
                            for (size_t i = 0; i < ARRAYSIZE(ct.oldIPs); i++)
                            {
                                ((BYTE*)(pHook->oldIPs))[i] = ((BYTE*)(ct.oldIPs))[i];
                            }


                            //memcpy(pHook->newIPs, ct.newIPs, ARRAYSIZE(ct.newIPs));

                            for (size_t i = 0; i < ARRAYSIZE(ct.newIPs); i++)
                            {
                                ((BYTE*)(pHook->newIPs))[i] = ((BYTE*)(ct.newIPs))[i];
                            }
                            ...
                            if (ct.patchAbove)
                            {
                                ...
                                for (size_t i = 0; i < sizeof(JMP_REL) + sizeof(JMP_REL_SHORT); i++)
                                {
                                    ((BYTE*)(pHook->backup))[i] = ((BYTE*)((LPBYTE)pTarget - sizeof(JMP_REL)))[i];
                                }
                            ...
                            else
                            {
                                ...
                                for (size_t i = 0; i < sizeof(JMP_REL); i++)
                                {
                                    ((BYTE*)(pHook->backup))[i] = ((BYTE*)pTarget)[i];
                                }
                            }
...
static unsigned int hde32_disasm(const void* code, hde32s* hs)
{
    ...
    for (size_t i = 0; i < sizeof(hde32s); i++)
    {
        ((BYTE*)hs)[i] = 0;
    }
